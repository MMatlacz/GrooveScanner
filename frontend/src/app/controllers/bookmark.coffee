angular.module 'upc'
  .controller 'BookmarkController', ($scope, $rootScope, $sce, $stateParams, toastr, Airport, Event, Plan) ->
    'ngInject'
    ...
    Event.get($stateParams.event_id).$promise.then (event) ->
      ...
      Airport.get($stateParams.airport).$promise.then (airport) ->
        ...
        Plan.query($stateParams.airport, event.place.location.city, event.place.location.country, start_time, end_time).$promise.then (data) ->
          $scope.plans = data
